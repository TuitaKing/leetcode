 ...
 */
public class MinClimbStairs_746 {
    public int minCostClimbingStairs_v1(int[] cost) {
        int oneStep=0,twoStep=0,res=0;
        for(int i = 2 ; i<=cost.length;i++){
            res=Math.min(oneStep+cost[i-1],twoStep+cost[i-2]);
            twoStep=oneStep;
            oneStep=res;
        }
        return oneStep;
    ...
    public int minCostClimbingStairs(int[] cost){
        ...
        db[0]=db[1]=0;// 可以跳两步，所以到第2层最大消耗都是0
        for(int i = 2 ; i<=cost.length;i++){
            db[i]=Math.min(cost[i-1]+db[i-1],cost[i-2]+db[i-2]);
        }
