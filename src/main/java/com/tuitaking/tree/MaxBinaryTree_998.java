 ...
 *
 * 如果插入的值比现在的根节点大，就将现在的作为根节点，原有root作为左子树，如果比现在节点小，插入现在的右子树相对的位置
 */
public class MaxBinaryTree_998 {
    public TreeNode insertIntoMaxTree(TreeNode root, int val) {
        ...
        TreeNode right=root;
        while (right.right!=null && right.right.val>val){
            right=right.right;
        }
        TreeNode node=right.right;
        TreeNode insertNode=new TreeNode(val);
        right.right=insertNode;
        insertNode.left=node;
        return  root;
    }
    public TreeNode insertIntoMaxTree_v2(TreeNode root, int val) {
        if(val > root.val){
            ...
            if(root.right == null){
                ...
                root.right = insertIntoMaxTree_v2(root.right, val);
            }
        }
        return root;
    }
